    ...
    def tojson(self, obj, check):
        ...
        for field in fields(obj):
            ...
            if is_dataclass(value):
                self.obj_dict[field.name] = self.tojson(value, False)
            elif isinstance(value, list):
                self.obj_dict[field.name] = [self.tojson(v, False) for v in value]
            elif isinstance(value, dict):
                self.obj_dict[field.name] = {k: self.tojson(v, False) for k, v in value.items()}
            elif isinstance(value, tuple):
                self.obj_dict[field.name] = tuple(self.tojson(v, False) for v in value)
            else:
...
#     recursion.tojson(actual_move)
    # f=open("learning_data11.json", "w", encoding="UTF-8")
    # f.writelines(recursion.tojson(actual_move))
    # f.close()
